...
const { data: PersonnesFilms, error } = await supabase
  .from('Personnes_Films')
  .select('id_films,Films(nom_film, affiche_film)')
  .eq('id_Personnes', props.id_Personnes)
        ...
        <div class="flex justify-evenly my-5 gap-5">
          <div v-bind="Filmographie" class="flex w-full justify-center flex-col text-start">
            <img
              ...
              :src="Filmographie.Films?.affiche_film"
              alt=""
            />
